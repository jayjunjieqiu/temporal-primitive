...
def plot_cluster_map(rep: str, x_pca: np.ndarray, labels: np.ndarray, centers: np.ndarray, k: int, fig_dir: Path) -> str:
    fig, ax = plt.subplots(figsize=(6.2, 5.4))
    sc = ax.scatter(x_pca[:, 0], x_pca[:, 1], c=labels, s=6, cmap="tab20", alpha=0.58, linewidths=0)
    ...
    ax.set_title(f"{DISPLAY_NAMES.get(rep, rep)}: PCA then KMeans, K={k}", fontsize=13)
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ...
    fig.tight_layout()
    out = fig_dir / f"{rep}_pca_kmeans_k{k}.png"
...
def plot_domain_map(
    ...
    ax.set_title(f"{DISPLAY_NAMES.get(rep, rep)}: PCA map colored by {label_key.replace('_', ' ')}", fontsize=13)
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ax.grid(color="#edf2f7", linewidth=0.7)
    handles = [
    ...
    out = fig_dir / f"{rep}_pca_{suffix}_labels.png"
    fig.savefig(out, dpi=260, bbox_inches="tight")
    plt.close(fig)
    return str(out)
...
def write_cluster_report(path: Path, outputs: dict[str, dict[str, Any]]) -> None:
    ...
    for rep in REPRESENTATIONS:
        ...
        lines += [
            ...
            "",
            f"![{display} PCA KMeans](../{Path(info['cluster_map']).relative_to(ROOT)})",
            "",
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs" / "pca_cluster_domain_reports")
    ...
    for rep in REPRESENTATIONS:
        ...
        x_pca, _scaler, _pca = fit_pca_space(data.embeddings[idx], args.seed)
        patches = data.patches[idx]
        k = BEST_K[rep]
        kmeans = KMeans(n_clusters=k, random_state=args.seed, n_init=20).fit(x_pca)
        labels = kmeans.labels_
        centers = kmeans.cluster_centers_

        cluster_map = plot_cluster_map(rep, x_pca, labels, centers, k, fig_dir)
        source_domain_map = plot_domain_map(rep, x_pca, metadata, "source_domain", SOURCE_ORDER, fig_dir)
        macro_domain_map = plot_domain_map(rep, x_pca, metadata, "macro_domain", MACRO_DOMAIN_ORDER, fig_dir)
        plot_center_nearest(
            rep,
            list(range(k)),
            x_pca,
            ...
            patches,
            fig_dir,
            output_name=f"{rep}_pca_kmeans_k{k}_center_nearest",
            title_suffix=f"K={k}",
        )
        prototype_panel = str(fig_dir / f"{rep}_pca_kmeans_k{k}_center_nearest.png")
    ...
    summary = {
        "objective": "PCA-space KMeans and domain-label overlay reports for selected Chronos-2 layers",
    ...
    (out_dir / "pca_cluster_domain_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    write_cluster_report(ROOT / "docs" / "97_pca_then_kmeans_cluster_report.md", outputs)
    write_domain_report(ROOT / "docs" / "98_pca_domain_label_report.md", outputs)
    print(json.dumps({"output_dir": str(out_dir), "reports": ["docs/97_pca_then_kmeans_cluster_report.md", "docs/98_pca_domain_label_report.md"]}, ensure_ascii=False))
