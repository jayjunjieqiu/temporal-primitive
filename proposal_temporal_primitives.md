...
经过最近一次和 Yuxuan Liang 老师讨论，当前阶段的研究问题需要进一步收敛到 **Chronos-2 的多层 patch representation 是否保留并重组 local temporal information**。这里有几个必须被实验直接回答的问题：

1. **Single patch independence**：单个 patch 在进入 TSFM 后，是否仍然保留相对独立的 local information，而不是完全被 whole context sentence 吞没？
2. **Layer effect**：`projection -> layer_0 -> layer_6 -> layer_11` 是否呈现从 local patch vocabulary 到 contextualized representation 的层间演化？老师的直觉是 early layers 更保留 spike、oscillation、trend 等 local information，middle layers 开始融合 context，top layer 更接近全局/上下文化语义。
3. **All-layer evidence**：不能只关注 `layer_0`。`projection`、`layer_0`、`layer_6`、`layer_11` 都要报告，因为它们分别对应 pre-contextual token representation、early local vocabulary、contextual mixing 和 more contextualized representation。
4. **K selection**：聚类的 K 不能只靠经验公式或视觉效果。需要用 K sweep、stability、cluster size、confounder NMI、original-space interpretability 和 macro-domain coverage 综合选择，并明确说明为什么最终 K setting 合理。
5. **Original-space semantics**：聚类结果必须回到原时序空间解释。老师想看到的是：以 KMeans center 为中心，展示离 center 最近的真实 patch examples，并判断这些 cluster 是否真的有 motif semantics。
6. **Cross-domain validity**：nearest examples 不能只来自 1-2 个 dataset。需要 macro-domain view 或 confidence-filtered macro-domain view，证明某些 cluster 是跨 Traffic / Energy / Environment / Finance / Health 等领域可复现的 temporal concepts，或诚实标记为 domain-specific / weak evidence。
7. **Human-prior motif 的角色**：`prior-guided motif` 可以作为 audit probe，但不能当 ground truth，也不能把它的名字直接当作 KMeans cluster 名称。老师会关心这些 human-prior motif 的数学定义和参考来源，因此必须把它们作为 probe 讲清楚，而不是作为最终 taxonomy。
8. **Evidence quality**：最终汇报不是“随便画一张好看的图”。主证据图需要经过预注册的质量闸门：稳定、不过碎、confounder 低、原空间可解释、跨 domain 有可信 match。失败案例和 weak match 应放在 diagnostic/appendix，而不是主结论。

因此，当前最合适的阶段性问题可以写成：

> For Chronos-2, do patch representations across projection, early, middle, and late layers preserve local temporal primitives, and how are these primitives reorganized into contextualized, cross-domain temporal concepts?
...
1. 先完成 Chronos-2-only 的 multi-layer cluster validation，覆盖 `projection`、`layer_0`、`layer_6`、`layer_11`。
